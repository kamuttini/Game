...
occurred. Therefore, you may want to setOrientation your `EXPECT_CALL` statements inside
...
`EXPECT_DEATH()` (in the extreme case, you want to setOrientation everything inside), or
...
`ADD_FAILURE*`) in constructors and destructors. The workaround is to setOrientation the
