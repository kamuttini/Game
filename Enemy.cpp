...
Enemy::Enemy(Player &player1, type &ID1, sf::Vector2f origin, sf::Vector2f bound) : player(player1),
    ...
    sprite = std::make_unique<Sprite>(setSprite(), *this, 2, 1, 3, 0);
    sprite->setScale(sf::Vector2f(1.9, 1.9));
    targetList.push_back(&player1);
...
void Enemy::move() {
    ...
    if (hp > 1 && distance < FOLLOW_RANGE) {
        strategy =std::make_unique<Follow>();

    } else {
        if (hp > 1) {
            strategy = std::make_unique<RandomMove>();
        } else {
            strategy =std::make_unique<Static>();
        }
    }
